...
# if python 2.7 or upper(like centos7),will use OrderedDict in python lib
# otherwise use from shadowsocks.ordereddict (for example centos 6)
try:
...
class LRUCache(collections.MutableMapping):
    ...
    def sweep(self, sweep_item_cnt=SWEEP_MAX_ITEMS, source='system'):
        # O(n - m)
        now = time.time()
        c = 0
        while c < sweep_item_cnt:
            if len(self._keys_to_last_time) == 0:
                break
            key = self._keys_to_last_time.keys()[0]
            logging.debug('%s ready to sweep key: %s', source, key)
            last_t = self._keys_to_last_time[key]
            ...
            del self._store[key]
            del self._keys_to_last_time[key]
            # self.close_callback(value) must put in the bottom ,
            # see ----> udprelay.py  del self._client_fd_to_server_addr[client.fileno()]
            # otherwise may cause ----> del self._store[key]     or  ------> del self._keys_to_last_time[key]
            #   failed , and cause memory leak
